...
AfishTransporter::AfishTransporter(vector loc) : npc(loc) {
	...
	//charImg = new Fimg(idleAnimPath + "spinNE.png", loc);
	//charImg->loadSurface("./images/character/character.png");
	//anim = new animation(.13, idleSE, charImg, true, loc);
	//anim->start();
	...
	anim = new animation("npc/fishTransporter/fishTransporterSpriteSheet.png", 30, 42, animData, true, loc);
	anim->spriteSheet->loadSurface("./images/npc/fishTransporter/fishTransporterSpriteSheet.png");
	...
	fishPileAnim = new animation("npc/fishTransporter/fishPileSpriteSheet.png", 30, 42, fishPileData, true, loc);
	fishPileAnim->setAnimation("emptyidleSE");
	fishPileAnim->start();

	fullnessText = new text("0/" + shortNumbers::convert2Short(maxHoldNum), "straight", loc, true, false, textAlign::center);
	...
	progressBar = new UprogressBar(true, 25, 3);
	progressBar->setVisibility(false);

	//setup("fishTransporter");
	setLoc(loc);
...
void AfishTransporter::startPathFinding() {
	if (world::autoFisherList.size() != 0 && autoFisherIndex != -1 && (Astar->stopped || Astar->finished))
		Astar->startPathFinding(loc, world::autoFisherList[autoFisherIndex]->loc + vector{ 0, 45 });
}

void AfishTransporter::draw(SDL_Renderer* renderer) {
	//__super::draw(renderer);

	anim->draw(renderer);
	fishPileAnim->draw(renderer);

	if (mouseOver(anim->spriteSheet) && fullnessText) {
		fullnessText->setLoc(loc + vector{ 0, -16 * stuff::pixelSize });
		fullnessText->draw(renderer);
		progressBar->setLoc(loc - vector{ progressBar->getSize().x / 2 / stuff::pixelSize, anim->cellHeight + 11.f });
	} else {
		progressBar->setLoc(loc - vector{ progressBar->getSize().x / 2 / stuff::pixelSize, anim->cellHeight + 3.f });
	}
	...
	if (canMove) {
		if (Astar->followingPath) {
			...
			if (autoFisherIndex != -1)
				collectTimer->start(calcCollectTimer(world::autoFisherList[autoFisherIndex]));
			else
...
vector AfishTransporter::calcGoTo(int autoFisherIndex) {
	if (autoFisherIndex != -1)
		return world::autoFisherList[autoFisherIndex]->loc + vector{ 0, 45 };
	return { 406, -159 };
}
...
void AfishTransporter::finishCollectTimer() {
	...
	if (autoFisherIndex != -1)
		collectFish(world::autoFisherList[autoFisherIndex]);
	else
		collectFish(NULL);
	...
	autoFisherIndex++;
	progressBar->setVisibility(false);
	if (autoFisherIndex >= world::autoFisherList.size() || calcCurrencyHeld() >= maxHoldNum) {
		autoFisherIndex = -1;
	}
...
void AfishTransporter::setLoc(vector loc) {
	...
	this->loc = loc;
	//if (anim && charImg)
	//	anim->setLoc(charImg->loc = loc - vector{charImg->w / 2.f, float(charImg->h)});
	if (anim) {
		anim->setLoc(loc - vector{ anim->cellWidth / 2.f, float(anim->cellHeight) });
		fishPileAnim->setLoc(anim->getLoc());
	}
}
...
void AfishTransporter::calcIdleProfits(float timeDiff) {
	if (world::autoFisherList.size() == 0)
		return;

	double totalCurrency = 0;
	for (int i = 0; i < world::autoFisherList.size(); i++)
		if (world::autoFisherList[i])
			totalCurrency += world::autoFisherList[i]->calcIdleProfits(timeDiff);

	// collection time per loop is maxCollectTime * autoFisher num
	float maxCollectTime = calcCollectTimer(NULL, true) * world::autoFisherList.size(); // if the autofishers aren't max this is wasting time

	// then calculate how long it takes to walk to each autofisher including selling
	vector start = calcGoTo(-1); // gets start loc
	float totalDist = math::distance(start, world::autoFisherList[0]->loc); // distances from start to 1
	for (int i = 1; i < world::autoFisherList.size() - 1; i++) {
		// loop through
		// get distance from 1 to 2, 2 to 3...
		totalDist += math::distance(world::autoFisherList[i]->loc, world::autoFisherList[i + 1]->loc);
	}
	// distance from last to start
	totalDist += math::distance(world::autoFisherList[world::autoFisherList.size() - 1]->loc, start);
	...
	double currency = 0;
	for (int i = 0; i < world::autoFisherList.size(); i++)
		currency += world::autoFisherList[i]->calcMPS() * loopTime * loopNum;
	...
	if (currency > totalCurrency) { // then collect totalCurrency
		...
		for (int i = world::autoFisherList.size() - 1; i >= 0; i--) {
			AautoFisher* currAutoFisher = world::autoFisherList[i];
