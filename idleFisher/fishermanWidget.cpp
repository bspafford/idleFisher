...
UfishermanWidget::UfishermanWidget(npc* parent) {
	...
	npcImg = new Image("./images/widget/npcbuttons/fisherman.png", {100, 100}, false);

	name = new text("asdfasdfasdf", "biggerStraight", { 0,0 });
	buffText = new text("asdfasdfasdf", "straight", { 0,0 });
	buffText->setTextColor(0, 255, 0);
	debuffText = new text("asdfasdfasdf", "straight", { 0,0 });
	...
	selectedIcon = new Image("./images/widget/selectedFisherIcon.png", { 0, 0 }, false);

	// fishing rod page
	fishingRodThumbnail = new Image("./images/widget/thumbnails/fishingRodThumbnail1.png", { 0, 0 }, false);
	fishingRodThumbnail->setSize(fishingRodThumbnail->getSize() * 2.f * stuff::pixelSize);
	powerText = new text("Power", "straight", { 0, 0 });
	powerUpgradeButton = new Ubutton(this, "widget/upgradeButton.png", 37, 16, 2, { 0, 0 }, false, false);
	powerUpgradeButton->addCallback(this, &UfishermanWidget::upgradePower);
	speedText = new text("Speed", "straight", { 0, 0 });
	speedUpgradeButton = new Ubutton(this, "widget/upgradeButton.png", 37, 16, 2, { 0, 0 }, false, false);
	speedUpgradeButton->addCallback(this, &UfishermanWidget::upgradeSpeed);
	catchChanceText = new text("Catch Chance", "straight", { 0, 0 });
	catchChanceUpgradeButton = new Ubutton(this, "widget/upgradeButton.png", 37, 16, 2, { 0, 0 }, false, false);
	catchChanceUpgradeButton->addCallback(this, &UfishermanWidget::upgradeCatchChance);

	powerButtonPrice = new text("0.00k", "straightDark", { 0, 0 }, false, false, textAlign::center);
	speedButtonPrice = new text("0.00k", "straightDark", { 0, 0 }, false, false, textAlign::center);
	catchChanceButtonPrice = new text("0.00k", "straightDark", { 0, 0 }, false, false, textAlign::center);

	powerLevelText = new text("0", "straight", { 0, 0 });
	speedLevelText = new text("0", "straight", { 0, 0 });
	catchChanceLevelText = new text("0", "straight", { 0, 0 });

	// fishing rod stats
	powerStatsText = new text("Power", "straight", {0, 0});
	powerStatsTextNum = new text("00", "straight", { 0, 0 }, false, false, textAlign::right);
	speedStatsText = new text("Speed", "straight", { 0, 0 });
	speedStatsTextNum = new text("00", "straight", { 0, 0 }, false, false, textAlign::right);
	catchChanceStatsText = new text("Catch Chance", "straight", { 0, 0 });
	catchChanceStatsTextNum = new text("00", "straight", { 0, 0 }, false, false, textAlign::right);
...
void UfishermanWidget::setup() {
	...
	for (int i = 0; i < SaveData::data.baitData.size(); i++) {
		...
		if (upgradeBox->buyButton)
			upgradeBox->buyButton->setParent(baitHolderList);
		baitHolderList->addChild(upgradeBox, upgradeBox->getSize().y);
	}
