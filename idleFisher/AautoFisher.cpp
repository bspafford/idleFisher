...
AautoFisher::AautoFisher(int id) {
	if (!autoFisherSpriteSheet)
		autoFisherSpriteSheet = new Image("./images/autoFisher/autoFisher.png", { 0, 0 }, true);
	if (!fishingLineSpriteSheet)
		fishingLineSpriteSheet = new Image("./images/autoFisher/fishingLine.png", { 0, 0 }, true);
	if (!outlineSpriteSheet)
		outlineSpriteSheet = new Image("./images/autoFisher/outline.png", { 0, 0 }, true);
	...
	autoFisher = new animation(autoFisherSpriteSheet, 42, 58, autoFisherData, true, loc);
	...
	outline = new animation(outlineSpriteSheet, 42, 58, outlineData, true, loc);
	...
	fishingLine = new animation(fishingLineSpriteSheet, 138, 139, fishingLineData, true, loc + vector{ 13, -74 });
...
// needs screen coords
bool AautoFisher::mouseOver(Image* img) {
	vector screenPos = math::worldToScreen(img->getLoc(), "topleft");

	vector min = vector{ screenPos.x, screenPos.y - img->getSize().y };
	vector max = vector{ screenPos.x + img->getSize().x, screenPos.y };
	...
	if (Main::mousePos.x >= min.x && Main::mousePos.x <= max.x && Main::mousePos.y >= min.y && Main::mousePos.y <= max.y) {
		...
		glm::vec4 pixelColor = math::GetPixelColor(img, (int)pos.x, (int)pos.y);
