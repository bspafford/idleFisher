...
Umap::Umap(UsailorWidget* parent) {
	...
	mapImg = new Image("./images/sailorMap.png", { 0, 0 }, false); // -1073, -739
	...
	for (int i = 0; i < worldButtonLoc.size() - 1; i++) {
		Image* line = new Image("./images/widget/maps/mapLine" + std::to_string(i + 1) + "-" + std::to_string(i + 2) + ".png", { 0, 0 }, false);
