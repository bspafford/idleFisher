...
void text::changeFont() {
	if (!SaveData::settingsData.pixelFont) {
		...
		normFont = new Image(Message, NULL, loc);
...
void text::loadTextImg() {
	if (textImg)
		delete textImg;
	textImg = new Image("./fonts/" + font + "/" + font + ".png", { 0, 0 }, useWorldPos);
...
void text::makeText(int i, std::string text, vector &offset) {
	...
	Rect* source = new Rect{ currInfo.loc.x, currInfo.loc.y, currInfo.size.x, currInfo.size.y };

	if (textString == "Feed Fish")
		std::cout << *source << std::endl;

	letters[i] = new Image(textImg, source, {0, 0}, false);
...
void text::setText(std::string text) {
	if (!SaveData::settingsData.pixelFont) {
		...
		textString = text;
		letters = std::vector<Image*>(text.size());// new Image* [textSize];
		...
		if (alignment != textAlign::left || isometric) {
			if (letters[0] && letters[letters.size() - 1]) {
				vector newLoc = { -letters[0]->getLoc().x, -letters[letters.size() - 1]->getLoc().y};
				for (int i = 0; i < letters.size(); i++) {
					letters[i]->setLoc((letters[i]->getLoc() + newLoc));
				}
			}
		}						
