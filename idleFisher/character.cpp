...
Acharacter::Acharacter() {
	...
	anim = new animation("character/characterSpriteSheet.png", 21, 49, animData, false, { 0, 0 });
	...
	fishingRod = new animation("character/fishingRod.png", 108, 83, fishingRodData, false);
	...
	bobberImg = new Image("./images/bobber.png", { 100, 100 }, false);
	bobberWaterOverlay = new Image("./images/bobberWaterOverlay.png", { 0, 0 }, false);
...
// catch fish
		...
		// set fish image
		if (fishImg)
			delete fishImg; // deletes old instance
		fishImg = new Image(currFish.thumbnail.c_str(), bobberLoc, false);
		showFish = true;
...
std::vector<std::vector<float>> Acharacter::calcFishProbability(std::vector<FfishData> fishData) {
	...
	for (int i = 1; i < fishData.size(); i++) {
		if (fishData[i].fishingPower <= upgrades::calcFishingRodPower() && (fishData[i].levelName == Main::currWorld || fishData[i].levelName == "premium")) {
