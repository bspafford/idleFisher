...
void Camera::Inputs(GLFWwindow* window, float deltaTime) {
	...
	/*
	// normalizes the movement, does this weird since coordiantes are isometric
	glm::vec3 newMove = glm::vec3(0);
	if (glm::length(move) != 0) {
		float x = move.x;
		float y = move.z;
		float h = 1.5;
		float k = h * 5.f / 6.f;
		float invLen = math::Q_rsqrt(k * x * x - h * x * y + k * y * y);
		newMove = glm::vec3(move.x * invLen, 0, move.z * invLen);
	}
	*/



	// move normally? so foward is (0, 1), and left is (-1, 0)
	// then after rotate the move?

	//newMove = scaleIsometricMovement3D(move);
	//float length = math::Q_sqrt(newMove.x * newMove.x + newMove.z * newMove.z);
	...
	//if (glm::length(rotated))
	//	std::cout << glm::length(rotated) << std::endl;

	//Position += speed * newMove * deltaTime;
	//Position += speed * scaleIsometricMovement3D(move) * deltaTime;
