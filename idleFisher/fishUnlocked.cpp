...
UfishUnlocked::UfishUnlocked() {
	bannerImg = new Image("./images/widget/fishUnlocked.png", { 0, 0 }, false);
...
void UfishUnlocked::start(FachievementStruct* achievementData) {
	...
	if (thumbnail)
		delete thumbnail;
	thumbnail = new Image("./images/widget/achievementIcons/achievementIcon" + std::to_string(achievementData->id + 1) + ".png", { 0, 0 }, false);
	if (name)
		delete name;
