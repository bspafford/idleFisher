...
AfishTransporter::AfishTransporter(vector loc) : npc(loc) {
	...
	//charImg = new Image(idleAnimPath + "spinNE.png", loc);
	...
	npcAnim = new animation("npc/fishTransporter/fishTransporterSpriteSheet.png", 30, 42, animData, true, loc);
...
void AfishTransporter::startPathFinding() {
	if (world::autoFisherList.size() != 0 && autoFisherIndex != -1 && (Astar->stopped || Astar->finished)) {
		Astar->startPathFinding(loc, world::autoFisherList[autoFisherIndex]->loc + vector{ 6, 21 });
	}
}
...
vector AfishTransporter::calcGoTo(int autoFisherIndex) {
	if (autoFisherIndex != -1)
		return world::autoFisherList[autoFisherIndex]->loc + vector{ 6, 21 };
	return world::bankSellLoc;
}
