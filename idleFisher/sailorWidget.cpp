...
UsailorWidget::UsailorWidget() {
	mapBackground = new Image("./images/widget/map.png", { 0, 0 }, false);
	...
	mapClosed = new Image("./images/widget/mapClosed.png", { 0, 0 }, false);

	std::unordered_map < std::string, animDataStruct> anim;
	anim.insert({ "open", { {0, 0}, {5, 0}, .1, false} });
	anim.insert({ "close", { {0, 1}, {5, 1}, .1, false} });
	mapAnim = new animation("widget/mapSpriteSheet.png", 402, 230, anim, false, { 0, 0 });
