...
glm::vec4 math::GetPixelColor(unsigned char* bytes, bool useAlpha, const int X, const int Y) {
...
glm::vec4 math::GetPixelColor(Image* img, const int X, const int Y) {
	const int x = X / stuff::pixelSize;
	const int y = Y / stuff::pixelSize;
	...
	*/
	...
	//glReadPixels(0, 0, 1, 1, img->textureFormat, GL_UNSIGNED_BYTE, &pixel);
	
	unsigned char* pixels = img->texture;
	int index = (y * img->ogW + x) * 4;
