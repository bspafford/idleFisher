...
void UprogressBar::draw(Shader* shaderProgram) {
	...
	vector newLoc = loc;
	background = new URectangle(newLoc, size, useCharLoc, backgroundColor);
	background->draw(shaderProgram);
	delete background;

	std::vector<URectangle*> foreground = std::vector<URectangle*>(percent.size());
	...
	for (int i = 0; i < percent.size(); i++) {
		...
		if (dir == FprogressBarDir::left) {
			...
			foreground[0] = new URectangle(vector{newLoc.x + size.x - temp, newLoc.y}, vector{float(temp), size.y}, useCharLoc, foregroundColor[i]);
		} else if (dir == FprogressBarDir::right) {
			// foreground = { int(newLoc.x), int(newLoc.y), int(size.x * percentVal), int(size.y) };
			foreground[i] = new URectangle(newLoc, {size.x * percentVal * (weight[i] / totalWeight), size.y }, useCharLoc, foregroundColor[i]);
			newLoc.x += size.x * percentVal * (weight[i] / totalWeight);

		} else if (dir == FprogressBarDir::up) {
			int temp = round(size.y * percentVal);
			foreground[0] = new URectangle({ newLoc.x, newLoc.y + size.y - temp }, { size.x, float(temp) }, useCharLoc, foregroundColor[i]);
		} else if (dir == FprogressBarDir::down)
			foreground[0] = new URectangle(newLoc, { size.x, size.y * percentVal }, useCharLoc, foregroundColor[i]);
