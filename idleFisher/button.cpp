...
Ubutton::Ubutton(widget* widget, std::string spriteSheetPath, int cellWidth, int cellHeight, int numberOfFrames, vector loc, bool useWorldLoc, bool useAlpha) {
	...
	buttonAnim = new animation(spriteSheetPath, cellWidth, cellHeight, animData, useWorldLoc, loc);
...
bool Ubutton::isMouseOver() {
	...
	if (useAlpha) {
		...
		if (mousePos.x >= min.x && mousePos.x <= max.x && mousePos.y >= min.y && mousePos.y <= max.y) {
			...
			glm::vec4 pixelColor = math::GetPixelColor(buttonAnim->spriteSheet, (int)pos.x, (int)pos.y);
