...
UfishComboWidget::UfishComboWidget() {
	...
	fishComboBorderImg = new Image(fishComboBorderPath, { 0, 0 }, false);
	fishComboBorderImg->setAnchor(anchor::center, anchor::center);
	fishImg = new Image(fishPath, { 0, 0 }, false);
	fishImg->setAnchor(anchor::center, anchor::center);

	greenRect = new URectangle({ 0, 0 }, { 0, 0 }, false, glm::vec4(0.4510, 1.0, 0.0, 1.0));
	greenRect->setAnchor(anchor::center, anchor::center);
	yellowRect = new URectangle({ 0, 0 }, { 0, 0 }, false, glm::vec4(0.8863, 1.0, 0.0, 1.0));
	yellowRect->setAnchor(anchor::center, anchor::center);
	backgroundRect = new URectangle({ 0, 0 }, fishComboBorderImg->getSize() - vector{ 4, 4 } *stuff::pixelSize, false, glm::vec4(0.3608, 0.4980, 0.6000, 1.0));
...
UfishComboWidget::~UfishComboWidget() {
	delete fishComboBorderImg;
	delete fishImg;
	delete greenRect;
	delete yellowRect;
	delete backgroundRect;
	fishComboBorderImg = nullptr;
	fishImg = nullptr;
	greenRect = nullptr;
	yellowRect = nullptr;
