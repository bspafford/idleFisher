...
void Usettings::draw(SDL_Renderer* renderer) {
	...
	saveText->draw(renderer);
	if (cancelButton)
		cancelButton->draw(renderer);
	cancelText->draw(renderer);
}
...
void Usettings::setupLocs() {
	...
	if (backButton) {
		...
		backButton->setLoc({ float(background->loc.x + (background->w - 4) * stuff::pixelSize - backButtonSize.x / 2), float(background->loc.y - backButtonSize.y / 2 + 4 * stuff::pixelSize) });
	}
	...
	if (cancelButton && background && saveButton)
		cancelButton->setLoc(background->loc + background->getSize() - saveButton->getSize() - vector{ 10, 10 } * stuff::pixelSize);
