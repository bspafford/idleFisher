...
double upgrades::calcFishComboSpeed(FfishData currFish, int quality) {
	...
	float tempSpeed = 1 / (SaveData::saveData.equippedFishingRod.power / 10) * currFish.fishSpeed;
...
double upgrades::calcMaxFishingInterval() {
	...
	double val = baseVal * Acharacter::fishTimeMultiplier * (1 - petBuffs::decreaseFishingIntervals()) * (1 - baitBuffs::decreaseFishIntervalDecraseFishCaught()[0]) * (1 - baitBuffs::increaseFishCaughtDecreaseFishIntervalsIncreaseCombo()[1]); // buff
	return val;
}
