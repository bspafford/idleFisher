...
buyAutoFisher::buyAutoFisher(vector loc) {
	...
	plusAnim = new animation("autoFisher/buyer/buyer.png", 26, 46, plusData);
...
void buyAutoFisher::draw(SDL_Renderer* renderer) {
	if (calcMaxAutoFishers() || !plusAnim)
		return;

	bool tempMouseOver = bMouseOver;
	bMouseOver = mouseOver();
	if (!tempMouseOver && bMouseOver) {
		plusAnim->setAnimation("hover");
		Main::hoverObject(NULL);
	} else if (tempMouseOver && !bMouseOver) {
		plusAnim->setAnimation("normal");
	...
	calcIfPlayerInFront();
	plusAnim->draw(renderer);
	priceText->draw(renderer);
}
...
void buyAutoFisher::spawnAutoFisher() {
	...
	if (calcMaxAutoFishers()) {
		...
		auto it = std::find(Main::allCollision.begin(), Main::allCollision.end(), collision);
		Main::allCollision.erase(it);
	}
}
...
bool buyAutoFisher::mouseOver() {
	...
	vector screenLoc = math::worldToScreen(plusAnim->getLoc());
	if (mousePos.x >= screenLoc.x && mousePos.x <= screenLoc.x + plusAnim->cellWidth * stuff::pixelSize && mousePos.y >= screenLoc.y && mousePos.y <= screenLoc.y + plusAnim->cellHeight * stuff::pixelSize)
		return true;
	return false;
}
...
void buyAutoFisher::updateLoc() {
	...
	if (plusAnim) {
		plusAnim->setLoc({ SaveData::data.autoFisherData[int(world::autoFisherList.size())].xLoc + 2, SaveData::data.autoFisherData[int(world::autoFisherList.size())].yLoc + 16 });
		priceText->setLoc(plusAnim->getLoc());
	}
...
void buyAutoFisher::calcIfPlayerInFront() {
	...
	vector screenLoc = math::worldToScreen(playerLoc);
	vector screenAutoLoc = math::worldToScreen(plusAnim ? plusAnim->getLoc() : vector{0, 0});
...
void buyAutoFisher::setupCollision() {
	...
	if (!collision) {
		...
		Main::allCollision.push_back(collision);
	} else {
		...
		auto it = std::find(Main::allCollision.begin(), Main::allCollision.end(), collision);
		if (it != Main::allCollision.end()) {
			int index = it - Main::allCollision.begin();
			delete Main::allCollision[index];
			collision = new Fcollision(points, " ");
			Main::allCollision[index] = collision;
		}
	}
}
